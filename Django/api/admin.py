...
class BikePlusRiderModelAdmin(admin.ModelAdmin):
    list_display = ['mass_rider', 'mass_bike', 'mass_other', 'crr',
                    'mechanical_efficiency', 'mol_whl_front', 'mol_whl_rear', 'wheel_radius']


@admin.register(CPModel)
class CPModelAdmin(admin.ModelAdmin):
    list_display = ['w_prime_recovery_function', 'below_steady_state_max_slope', 'below_steady_state_power_usage',
                    'over_threshold_min_slope', 'over_threshold_power_usage', 'steady_state_power_usage']


@admin.register(PositionModel)
class PositionModelAdmin(admin.ModelAdmin):
    list_display = ['climbing_cda_increment', 'climbing_min_slope',
                    'descending_cda_increment', 'descending_max_slope']
...
class StaticModelAdmin(admin.ModelAdmin):
    list_display = ['bike_plus_rider_model', 'cp_model',
                    'position_model', 'environment_model', 'technical_model']
